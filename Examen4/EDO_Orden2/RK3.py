...
def RK3(f,a,b,y0,z0,n):
    ...
    for k in range(n):
        ...
        K1_y = z_v[k]
        K1_z = f(x_v[k],y_v[k],z_v[k])
        K2_y = z_v[k] + (h/2)*K1_z
        K2_z = f(x_v[k]+h/2,y_v[k]+(h/2)*K1_y,z_v[k]+(h/2)*K1_z)
        K3_y = z_v[k] +h*(-K1_z+2*K2_z)
        K3_z = f(x_v[k]+h,y_v[k]-h*(-K1_y+2*K2_y),z_v[k]+h*(-K1_z+2*K2_z))

        yKN = y_v[k] + (h/6)*(K1_y+4*K2_y+K3_y)
        zKN= z_v[k] + (h/6) * (K1_z+4*K2_z+K3_z)
...
plt.plot(x, y_aprox, 'r', label='Método de RK3 2do Orden',linewidth=1)
